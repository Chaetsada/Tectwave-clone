...
const page = async ({ params }: any) => {
  ...
  const filteredData = articlesData.filter(
    (item: ArticleProps) => item.link === `/articles/${params.slug}`
  );

  return (
    <main className="min-h-screen py-[80px] px-[24px] md:px-[80px]">
      {filteredData.map((item:ArticleProps) => (
        <div key={item.id} className="w-full lg:w-[80%] mx-auto flex flex-col gap-5 lg:gap-14">
